      ...
      setTimeout(function () {
        if (message.reactions.get('🎉').count <= 1) {
          ...
          let embed3 = new Discord.RichEmbed()
            ...
            .setFooter('Made by Sunny Singh||Ended At')
            .setColor(color)
            //.setFooter(`${copunt} Winner(s)`)
            .setTimestamp();
          message.edit(embed3)
          message.channel.send(`${tada}-Congrats to ${winners.filter(u => u !== undefined && u !== null).map(u => u.toString()).join(", ")} with winning ${question}`)
        }
      }, time);
    })
  })
